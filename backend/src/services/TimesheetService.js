...
const { normalizeToMonday } = require('../utils/dateHelpers');
...
class TimesheetService {
  ...
  async submitTimesheet(dto, employeeId) {
    ...
    const allocatedProjectIds = activeAllocations.map(
      (a) => (a.projectId?._id || a.projectId).toString()
    );
  ...
  /**
   * Get an employee's timesheet history.
   */
  ...
  /**
   * Get a specific timesheet for an employee and week.
   */
  ...
  /**
   * Get team timesheets for a manager (for a specific week).
   * Returns timesheets for all employees allocated to the manager's projects.
   */
  async getTeamTimesheets(managerId, weekStart) {
    const normalizedWeekStart = normalizeToMonday(new Date(weekStart));

    // Find all active allocations for manager's projects
    ...
    const employeeIds = [...new Set(
      managerAllocations.map((a) => (a.employeeId?._id || a.employeeId).toString())
    )];
